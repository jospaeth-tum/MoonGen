...
function configure(parser)
	...
	parser:option("-S --pos-sync", "If set, synchronize with pos_sync before sniffing and starting the timeout.")
	parser:option("-T --pos-sync-tag", "The pos_sync tag to synchronize on."):args(1):default("default")
...
function master(args)
	if args.debug then
		...
		-- synchronize with pos_sync if the flag is set
		if args.pos_sync then
			if args.pos_sync_tag == "default" then
				os.execute("pos_sync")
			else
				os.execute("pos_sync -t " .. args.pos_sync_tag)
			end
		end
